...
function decodeAbi(data: string): Decoded | null {
  // data starts with 0x + 8 hex selector + word-aligned (64-hex-char) static slots
  ...
  if (selector === SELECTOR_ATTEST) {
    ...
    const signalOffset    = Number(hexToBigInt(word(4)));            // dynamic ptr
    const confidence      = hexToBigInt(word(5)).toString();
    const isValid         = hexToBigInt(word(6)) === 1n;
    // Decode signal string at offset (offset is in bytes from start of payload)
          ...
          <div style={{ padding: "20px 24px" }}>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              {logs.length > 0 && (
                <button
                  onClick={() => {
                    // try to seed with a recent tx hash from logs detail
                    const txLog = logs.find(l => l.detail?.includes("tx ") || l.detail?.includes("0x"));
                    const m = txLog?.detail?.match(/0x[a-fA-F0-9]{64}/);
                    if (m) setVerifyInput(m[0]);
                    else setVerifyInput("");
                  }}
                  style={{
                    padding: "12px 16px", background: "transparent", border: "1px solid var(--border)",
                    borderRadius: 4, fontFamily: "var(--font-hud), monospace", fontSize: 15,
                    color: "var(--text-dim)", cursor: "pointer", letterSpacing: "0.08em",
                  }}
                  title="Try a sample tx from the live log"
                >SAMPLE</button>
              )}
            </div>
