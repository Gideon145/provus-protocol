...
> **PROVUS is a verifiable signal engine on 0G mainnet � 80,000+ autonomous attestations prove that an AI trading strategy actually ran, when it ran, and what it saw.**

**0G Stack used:** Chain (mainnet, 80k+ TXs) � Compute (Qwen-2.5-7B TEE inference) � Storage (Merkle archives) � **3 of 5 components**
...
[![Judge Mode](https://img.shields.io/badge/Judge%20Mode-No%20wallet%20needed-ff9500)](https://provus-protocol-frontend.vercel.app/judge)
...
[![80,000+ TXs](https://img.shields.io/badge/Mainnet%20TXs-80%2C790%2B-green)](https://chainscan.0g.ai/address/0x94A4365E6B7E79791258A3Fa071824BC2b75a394)
...
> ### ?? **For judges: open [`/judge`](https://provus-protocol-frontend.vercel.app/judge) � one page, no wallet, live mainnet nonce.**
>
> Or follow the longer walkthrough in [JUDGE_GUIDE.md](./JUDGE_GUIDE.md).

---

## Fast Review Path (30 seconds)
...
**80,790+ confirmed transactions** on 0G Chain mainnet (agent wallet `0x94A4...A394`, lifetime nonce, persistent across restarts) form an auditable track record that *precedes* any capital deployment.
...
|---|---|---|---|
| **Mainnet (primary)** | 16661 | 0G **Chain** | 80,790+ confirmed TXs on agent wallet � irrefutable proof of long-running autonomous operation |
...
> This section addresses the core hackathon requirement: *"Clear proof that at least one 0G core component has been integrated."* PROVUS leads with **0G Chain mainnet** as the verified integration (80k+ TXs), and additionally runs a **parallel testnet deployment that exercises all three 0G components end-to-end** (Chain + Compute + Storage). See *What's Live vs. Implemented* below.

### ? Component 1: 0G Chain (Mainnet) � **Live, 80k+ TXs**
...
| **Agent Wallet (mainnet)** | `0x94A4365E6B7E79791258A3Fa071824BC2b75a394` | [**80,790+ confirmed TXs**](https://chainscan.0g.ai/address/0x94A4365E6B7E79791258A3Fa071824BC2b75a394) |
...
# -> {"result":"0x13b96"}  (= 80,790 lifetime confirmed transactions, growing)
```
...
|---|---|---|
| `recordVolatility()` every 60s on 0G **mainnet** | **LIVE** | [Agent wallet TXs](https://chainscan.0g.ai/address/0x94A4365E6B7E79791258A3Fa071824BC2b75a394) � 80k+ |
...
| Agent Wallet (mainnet) | https://chainscan.0g.ai/address/0x94A4365E6B7E79791258A3Fa071824BC2b75a394 | **80,790+ confirmed TXs** |
...
|---|---|---|---|
| On-Chain | Lifetime confirmed TXs (agent wallet) | **80,790+** | `eth_getTransactionCount` on `0x94A4�A394` |
...
| TEE SDK | `@0glabs/0g-serving-broker` | ??? 0G Compute | Implemented |
...
**Step 1 � Confirm 80k+ mainnet TXs (30s)**
```bash
curl -X POST https://evmrpc.0g.ai \
  -H 'content-type: application/json' \
  -d '{"jsonrpc":"2.0","id":1,"method":"eth_getTransactionCount","params":["0x94A4365E6B7E79791258A3Fa071824BC2b75a394","latest"]}'
...
|---|---|---|
| Lifetime mainnet TXs | 80,000+ | `curl -X POST https://evmrpc.0g.ai -H 'content-type: application/json' -d '{"jsonrpc":"2.0","id":1,"method":"eth_getTransactionCount","params":["0x94A4365E6B7E79791258A3Fa071824BC2b75a394","latest"]}'` |
...
|---|---|---|
| **0G Chain** | ? Live � 80,790+ confirmed TXs (lifetime nonce on agent wallet) | ? Live � `recordVolatility()` + `attest()` every 60s |
...
PROVUS does not fake liveness. Every degraded path is surfaced, never papered over � and the 80,000+ TX trail is the proof these recovery paths work in production, not in theory.
